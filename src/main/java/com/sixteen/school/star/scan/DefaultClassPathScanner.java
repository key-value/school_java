...
public class DefaultClassPathScanner extends ClassPathBeanDefinitionScanner {
    ...
    protected void registerFilters() {
      this.addIncludeFilter(new AnnotationTypeFilter(MultiSign.class));
...
//        });
    ...
    void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
        ...
        for (BeanDefinitionHolder holder : beanDefinitions) {
            ...
            //依赖注入
            definition.getPropertyValues().add("interfaceClass", mapperClassName);
            // 根据工厂的名称创建出 默认的BaseMapper实现
            definition.getPropertyValues().add("multiBaseService", new RuntimeBeanReference(this.mapperManagerFactoryBean));
            definition.setBeanClass(BaseServiceFactoryBean.class);
